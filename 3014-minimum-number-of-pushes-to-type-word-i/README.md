...
### Intuition
Because all letters in the input word are distinct, each letter appears exactly once. To minimize the total number of pushes, we should greedily assign the first 8 letters to the first position of the 8 available keys (keys 2 through 9), the next 8 letters to the second position, and so on.

### Approach
- Determine the length of the input word, which represents the total number of distinct characters we need to map.
- Initialize a `pushes` counter to keep track of the total cost.
- Iterate through each character's index `i` from `0` to `n - 1`.
- For each character, calculate its push cost using the formula `(i / 8) + 1` and add it to the total. This naturally groups the characters: indices 0-7 cost 1 push, 8-15 cost 2 pushes, 16-23 cost 3 pushes, and so forth.
- Return the accumulated total of pushes.

### Complexity
- **Time:** O(N) where N is the length of the word, as we iterate exactly N times (effectively O(1) since N ≤ 26).
- **Space:** O(1) because we only use a few integer variables for counting, requiring constant extra space.
