...
### Intuition
The key insight is to treat the Roman numeral conversion as a greedy "coin change" problem. By explicitly including the six special subtractive forms (like IV for 4 and CM for 900) alongside the standard symbols in descending order, we can repeatedly subtract the largest possible value from our number and append its corresponding symbol until the number reaches zero.

### Approach
- Create two parallel arrays: one holding the integer values and another holding their corresponding Roman numeral strings.
- Ensure these arrays are sorted in descending order and include both standard symbols and the special subtractive cases (e.g., 900 mapped to "CM", 4 mapped to "IV").
- Initialize a `StringBuilder` to efficiently construct the final Roman numeral string.
- Iterate through the values array from largest to smallest.
- While the remaining `num` is greater than or equal to the current value, subtract the value from `num` and append the corresponding symbol to the string builder.
- Return the constructed string once the iteration is complete.

### Complexity
- **Time:** O(1) — The maximum input is 3999, meaning the loops will execute a strictly bounded, constant number of times (at most 15 subtractions).
- **Space:** O(1) — The parallel arrays are of fixed size (13 elements) and the output string length is bounded (maximum 15 characters), requiring constant extra memory.
