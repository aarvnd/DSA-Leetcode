...
### Intuition
The shortest beautiful substring containing exactly $k$ '1's must both start and end with a '1', otherwise, we could trim leading or trailing '0's to get a shorter valid string. By identifying the exact positions of all '1's in the string, we can limit our search to only the substrings bounded by the $i$-th and $(i+k-1)$-th '1'.

### Approach
- Scan the input string and store the index of every '1' character in a list.
- If the total number of '1's found is less than $k$, immediately return an empty string since no valid substring exists.
- Iterate through the list of indices, looking at windows of size $k$ (from the $i$-th '1' to the $(i + k - 1)$-th '1').
- For each window, extract the substring from the original string and calculate its length.
- If this length is strictly smaller than the minimum length seen so far, update the minimum length and store this substring as the best answer.
- If the length equals the minimum length, use string comparison to keep the lexicographically smaller substring.

### Complexity
- **Time:** O(N^2) — where N is the length of the string; iterating through the windows takes O(N) steps, and extracting/comparing substrings takes O(N) time per step.
- **Space:** O(N) — for storing the list of '1' indices and the memory allocated for the substring candidates.
